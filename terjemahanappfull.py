...
class Translator:
    ...
    def tokenizing(
        ...
        allowedChar = (
            "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789âèḍ '-.,"
        )
    ...
    def translate(self, hasil_NER, kalimat, dictionary):
                ...
                if term == ".":
                    ...
                    if term == "ghallu":
                        ...
                        if "-" in term:
                            ...
                            self.lemma = temp["kd"]
                            makna_SLA = self.modified_WSD(
                                temp["kd"], kalimat, dictionary
                            )
                            hasil += (
                                temp["prefix"]
                                + makna_SLA
                            ...
                            if term not in dictionary.keys():
                                ...
                                hasil += (
                                    self.modified_WSD(term, kalimat, dictionary) + " "
                                )
                                self.dic = True
                                self.lemma = term
            else:
    ...
    def modified_WSD(self, token, kalimat, dictionary):
        ...
        if data_ambigu["madura"].isin([token]).any():
            ...
            # print(nilai_ditemukan)
            hasil = self.SLA(kalimat, nilai_ditemukan, dictionary[token][0])
        else:
    ...
    def SLA(self, kalimat, data_ambigu, makna_awal):
...
def NER_location(kalimat):
    ...
    data_wilayah = data["nama"].values
    # print(data_wilayah)
    preposisi = ["è", "è", "ka", "ḍâ'", "neng", "ḍâri", "sampè"]
    loc = [
        "alamat",
        "dhisa",
        "koṭṭa",
    ...
    obj = Translator()
    tokens = obj.ghalluIdentification(obj.ceIdentification(obj.tokenizing(kalimat)))
    # print(kalimat)
    # print("token : ",tokens)
    # tokens = kalimat.split()
    bigram = c_bigram(tokens, kalimat)
    trigram = c_trigram(tokens, kalimat)
    dic = []
    for token in tokens:
        dic.append([token.lower(), None])
    # print(dic)

    for i, elemen in enumerate(trigram):
        # print("elemen 0", elemen[0])
        if i > 0:
            if tokens[i - 1] in preposisi or tokens[i - 1] in loc:
                ...
                elif elemen.upper() in data_wilayah:
                    for j in range(i, i + 3):
                        # dic[tokens[j].lower()] = "loc"
                        dic[j][1] = "loc"
            ...
            if elemen == elemen.title():
                ...
                for j in range(i, i + 3):
                    ...
                    dic[j][1] = "loc"
                # print('wilayah',elemen)
    for i, elemen in enumerate(bigram):
        # print("elemen 0", elemen[0])
        if i > 0:
            if tokens[i - 1] in preposisi or tokens[i - 1] in loc:
                if elemen == elemen.title():
...
if tombol_trans:
    ...
    if user_input == "":
        ...
        hasil_NER = NER_location(input_awal)
        obj = Translator()
        result = obj.translate(hasil_NER, input_awal, kamus)
